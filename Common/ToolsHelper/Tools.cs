...
namespace Common
{
    ...
    public static class Tools
    {
        ...
        {
            string enumText = "";
            FieldInfo[] fieldinfos = enumValue.GetType().GetFields();
            foreach (FieldInfo field in fieldinfos)
            {
                if (field.Name == enumValue.ToString())
                {
                    Object[] objs = field.GetCustomAttributes(typeof(DescriptionAttribute), false);
                    enumText = ((DescriptionAttribute)objs[0]).Description;
                }
            }
            return enumText;
        }
